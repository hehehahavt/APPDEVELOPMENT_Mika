...
void displayWAVHDR(WAVHDR hdr){
	...
	gotoXY(1,10); setFGcolor(CYAN); printf("%.2f bit/s", hdr.BitsPerSample);
	gotoXY(1,20); setFGcolor(YELLOW); printf("%.2f sps", hdr.SampleRate);
#endif
}
...
void displayWAVdata(short int d[]){
	...
	for(i=0; i<80; i++){
		sum200 = 0.0; 
		for(j=0; j<SAMPLE_RATE/80; i++){
			sum200 += (*d)*(*d);
			d++; // treat d as a pointer to make stuff easier
		}
